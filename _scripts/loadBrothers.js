...
function loadJSON(callback) {
  ...
  xobj.open('GET', '{{ site.baseurl }}/_scripts/brotherdata.json', true);
  ...
  for (var grade in dictionary) {
    ...
    for (var name in dictionary[grade]) {
      if (dictionary[grade][name]['linkedin']) {
        html += "<div class='col-6 col-sm-3 col-lg-2 headshot'><a target='_blank' href='"+dictionary[grade][name]['linkedin']+"'><img class='linkedin' src={{site.baseurl}}/_assets/svg/linkedin.svg></a><img class='headshot' src='"+dictionary[grade][name]['headshot']+"'/><h4>"+name+"</h4></div>"
      } else {
